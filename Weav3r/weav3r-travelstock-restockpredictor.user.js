...
(function () {
	...
	// Read the "Carry" (items per flight) value from the trip filter panel.
	function getCarryCount() {
		const input = document.querySelector('input[aria-label="Items per flight"]');
		if (!input) return null;
		const val = parseInt((input.value || '').replace(/[^\d]/g, ''), 10);
		return isNaN(val) || val <= 0 ? null : val;
	}

	// Read the item's "Shop" price from the table row directly above the
	// expanded detail panel — the first "$"-formatted ts-num (Stock has none).
	function getShopPrice(panel) {
		const row = panel.closest('tr');
		const prevRow = row?.previousElementSibling;
		if (!prevRow) return null;
		for (const el of prevRow.querySelectorAll('.ts-num[title]')) {
			const title = el.getAttribute('title') || '';
			if (title.startsWith('$')) {
				const val = parseInt(title.replace(/[^\d]/g, ''), 10);
...
function findSelloutInChartData(data, ticks) {
	...
	function processPanel(panel) {
		...
		const carryCount = getCarryCount();
		const shopPrice = getShopPrice(panel);
		const bringAmount = (carryCount != null && shopPrice != null) ? carryCount * shopPrice : null;
		...
		if (travelMinutes != null && sellOutMinutes != null) {
			...
			if (maxStock && !isNaN(rateHigh)) {
				...
				if (arrivesWithStock) {
					...
					${bringAmount != null ? `
					<p style="font-size:9px;color:var(--text-secondary);margin:4px 0 1px 0;">Bring: <span style="color:${color};font-weight:600;">$${bringAmount.toLocaleString()}</span> (${carryCount} × $${shopPrice.toLocaleString()})</p>` : ''}
	...
	// This site is a Next.js SPA: navigating here from elsewhere on weav3r.dev
	// uses history.pushState rather than a real page load, so a userscript
	// manager's @match-based injection never fires for it and the script would
	// otherwise only start working after a manual refresh. We match the whole
	// site and gate all work on the current pathname instead, re-checking
	// whenever pushState/replaceState/popstate fires.
	function isTravelStockPage() {
		return location.pathname.startsWith('/travel-stock');
	}

	let debounce;
	function scheduleCheck() {
		if (!isTravelStockPage()) return;
	...
	window.addEventListener('locationchange', scheduleCheck);
